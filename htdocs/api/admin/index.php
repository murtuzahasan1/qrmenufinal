...
class AdminAPI {
    ...
    // ... (The rest of the AdminAPI class and its methods remain exactly the same as before)
    ...
    private function handleOrders($method) {
        ...
        switch ($method) {
            case 'GET':
                $restaurantId = $_GET['restaurant_id'] ?? $currentUser['restaurant_id'];
                $branchId = $_GET['branch_id'] ?? $currentUser['branch_id'];
                $status = $_GET['status'] ?? null;
                
                $sql = "SELECT o.*, b.name as branch_name FROM orders o JOIN branches b ON o.branch_id = b.id WHERE o.restaurant_id = ?";
                $params = [$restaurantId];
